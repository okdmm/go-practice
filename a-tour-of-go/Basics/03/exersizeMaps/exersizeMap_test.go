...
func TestWordCountFunc(t *testing.T) {
	...
	for _, test := range tests {
		...
		if len(got) != len(test.expected) {
			t.Fatalf("WordCount has wrong length. expected = %d, got =%d", len(test.expected), len(got))
		}
		for key := range got {
			if got[key] != test.expected[key] {
				t.Fatalf("wrong value. expected = %d, got=%d", test.expected[key], got[key])
			}
		}
	}
}
